    ...
    <div className="relative aspect-[3/4] w-full overflow-hidden bg-white mb-6 border border-[#611F3A]/10 shadow-sm rounded-sm group/carrossel">
      <div className="flex h-full w-full transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${fotoAtual * 100}%)` }}>
        {imagens.map((img, index) => (
          <img key={index} src={img} alt={`${nome} - ${index}`} className="w-full h-full object-cover flex-shrink-0" />
        ))}
      </div>
      {imagens.length > 1 && (
        <>
          <button onClick={fotoAnterior} className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/70 text-[#611F3A] p-2 rounded-full text-xs md:opacity-0 md:group-hover/carrossel:opacity-100 transition-opacity">❮</button>
          <button onClick={proximaFoto} className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/70 text-[#611F3A] p-2 rounded-full text-xs md:opacity-0 md:group-hover/carrossel:opacity-100 transition-opacity">❯</button>
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 p-1.5 bg-black/5 rounded-full backdrop-blur-sm">
            {imagens.map((_, index) => (
              <button key={index} onClick={() => setFotoAtual(index)} className={`w-1.5 h-1.5 rounded-full transition-all ${index === fotoAtual ? 'bg-[#D4AF37] scale-110' : 'bg-white/60'}`} />
            ))}
          </div>
        </>
      )}
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#E2AFC1] text-[#611F3A] font-sans relative overflow-x-hidden">
      ...
      {/* NOTIFICAÇÃO (TOAST) - Prioridade Máxima */}
      <div className={`fixed top-24 left-1/2 -translate-x-1/2 w-[85%] md:w-auto bg-white/95 backdrop-blur-md text-[#D4AF37] px-8 py-5 rounded-2xl shadow-2xl z-[9999] transform transition-all duration-500 border border-[#D4AF37]/30 ${notificacao ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'}`}>
        <p className="text-xs md:text-sm uppercase tracking-widest font-bold text-center italic font-serif leading-tight">✨ {notificacao}</p>
      </div>
      ...
      <section className="bg-white px-6 py-16 md:py-24 text-center border-b border-[#611F3A]/5">
        ...
        <h2 className="text-4xl md:text-7xl font-extralight text-[#611F3A] mb-8 leading-tight italic font-serif">A elegância que <br /> <span className="text-[#D4AF37]">você merece.</span></h2>
        <button className="bg-[#611F3A] text-white px-10 py-4 rounded-full text-xs uppercase tracking-[0.2em] font-bold hover:bg-[#D4AF37] transition-all shadow-lg active:scale-95">Conferir Lançamentos</button>
      </section>

      {/* Barra de Benefícios */}
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-0 bg-white/50 py-8 border-b border-[#611F3A]/10 text-center">
        <div className="flex flex-col items-center"><span className="text-lg mb-1">💳</span><p className="text-[10px] uppercase font-bold tracking-widest">Até 6x sem juros</p></div>
        <div className="flex flex-col items-center border-y md:border-y-0 md:border-x border-[#611F3A]/10 py-4 md:py-0"><span className="text-lg mb-1">🚚</span><p className="text-[10px] uppercase font-bold tracking-widest">Frete Grátis acima de R$ 399</p></div>
        <div className="flex flex-col items-center"><span className="text-lg mb-1">✨</span><p className="text-[10px] uppercase font-bold tracking-widest">Curadoria Exclusiva</p></div>
      </section>
            ...
            <div key={produto.id} className="flex flex-col group">
              ...
              <div className="text-center">
                ...
                <div className="flex flex-col gap-3">
                  <button onClick={() => adicionarAoCarrinho(produto)} className="bg-[#611F3A] text-[#D4AF37] py-4 rounded-lg text-[10px] uppercase tracking-[0.2em] font-bold shadow-md active:scale-95 transition-all w-full">Adicionar à Sacola</button>
                  <button onClick={() => setGuiaAberto(true)} className="text-[9px] uppercase tracking-widest text-[#611F3A]/40 font-bold underline underline-offset-4 hover:text-[#611F3A] transition-colors">Guia de Medidas</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {/* Carrinho Lateral */}
      <div className={`fixed top-0 right-0 h-full w-full md:w-96 bg-white shadow-2xl z-[1000] transform transition-transform duration-500 p-8 border-l border-[#611F3A]/10 ${carrinhoAberto ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="space-y-6 overflow-y-auto max-h-[60vh] pr-2">
          {carrinho.map((item, index) => (
            <div key={index} className="flex gap-4 border-b border-[#611F3A]/5 pb-4 items-center">
              ...
              <button onClick={() => removerDoCarrinho(index)} className="p-2 text-[#611F3A]/20 hover:text-[#611F3A] transition-colors">✕</button>
            </div>
          ))}
        ...
        {carrinho.length > 0 && (
          <div className="absolute bottom-8 left-8 right-8 bg-white pt-6 border-t border-[#D4AF37]/20">
            ...
            <button onClick={finalizarPedidoWhatsApp} className="w-full bg-[#611F3A] text-white py-5 rounded-xl text-[10px] uppercase tracking-[0.3em] font-bold shadow-xl active:scale-95 transition-all">Finalizar no WhatsApp</button>
          </div>
        )}
      </div>
