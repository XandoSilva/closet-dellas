...
function ProdutoCard({ produto, categoriasBase, adicionarAoCarrinho, setNotificacao, abrirDetalhe }: any) {
  ...
  return (
    <div className="flex flex-col group animate-in fade-in duration-500 relative">
      {esgotado && (
        <span className="absolute top-3 left-3 bg-zinc-500 text-white text-[8px] uppercase font-bold px-2.5 py-1.5 rounded-sm z-10 shadow-md">Esgotado</span>
      )}
      {!esgotado && produto.estoqueTotal === 1 && (
        <span className="absolute top-3 left-3 bg-[#D4AF37] text-white text-[8px] uppercase font-bold px-2.5 py-1.5 rounded-sm z-10 shadow-md animate-pulse">Última Peça</span>
      )}
      <div className="relative aspect-[3/4] w-full rounded-md overflow-hidden group/imagem shadow-sm">
        ...
        <div className="absolute inset-0 bg-black/10 opacity-0 group-hover/imagem:opacity-100 transition-opacity pointer-events-none md:flex items-center justify-center hidden">
          <button onClick={(e) => { e.stopPropagation(); abrirDetalhe(produto); }} className="bg-white/95 text-[#611F3A] px-6 py-3 rounded text-[10px] uppercase font-bold shadow-xl pointer-events-auto hover:bg-[#611F3A] hover:text-white transition-all transform translate-y-4 group-hover/imagem:translate-y-0">Ver Detalhes</button>
        </div>
      </div>
      <div className="text-left mt-3">
          ...
          {produto.grade.map((item: any) => (
            <button 
              ...
              className={`w-7 h-7 rounded-full text-[9px] font-bold border flex items-center justify-center transition-all ${
                item.qtd <= 0 ? 'opacity-20 cursor-not-allowed' :
                tamanho === item.tam ? 'bg-[#611F3A] text-white border-[#611F3A]' : 'bg-white text-zinc-600 border-zinc-200 hover:border-[#611F3A]'
              }`}
            >
              {item.tam}
        ...
        <div className="flex gap-2">
          <button onClick={() => abrirDetalhe(produto)} className="md:hidden flex-1 bg-zinc-900 text-white py-3 rounded-md text-[9px] uppercase font-bold">Ver Detalhes</button>
          <button onClick={handleAddCart} disabled={esgotado} className="flex-1 bg-[#611F3A] text-white py-3 rounded-md text-[9px] uppercase font-bold shadow-md disabled:bg-zinc-200 transition-colors">Add Sacola</button>
        </div>
      </div>
    </div>
  );
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-white text-zinc-900 font-sans relative overflow-x-hidden">
      ...
      {notificacao && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 bg-white px-8 py-4 rounded-full shadow-2xl z-[9999] border border-[#D4AF37] animate-in fade-in slide-in-from-top-4">
          <p className="text-xs font-bold text-[#611F3A]">✨ {notificacao}</p>
        </div>
      )}

      {/* NAV COM BUSCA INTELIGENTE (CÓDIGO DE BARRAS) */}
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex gap-4">
            ...
            <button className="bg-[#611F3A] text-white px-5 py-2.5 rounded-full font-bold text-xs flex items-center gap-2">
              👜 <span className="hidden md:inline">Sacola</span> <span>{carrinho.length}</span>
            </button>
          </div>
        </div>
      </nav>
      ...
      <section className="max-w-7xl mx-auto py-12 px-6">
        <div className="flex flex-wrap gap-3 mb-12 justify-center md:justify-start">
           <button onClick={() => {setCategoriaAtiva('todas'); setSubCategoriaAtiva(null);}} className={`px-6 py-2.5 rounded-full text-[10px] font-bold border transition-all ${categoriaAtiva === 'todas' ? 'bg-[#611F3A] text-white shadow-md' : 'bg-white text-zinc-600 border-zinc-200'}`}>TODAS</button>
           {categoriasBase.map(cat => (
             <div key={cat.id} className="relative group/menu">
                <button onClick={() => {setCategoriaAtiva(cat.id); setSubCategoriaAtiva(null);}} className={`px-6 py-2.5 rounded-full text-[10px] font-bold border transition-all ${categoriaAtiva === cat.id ? 'bg-[#611F3A] text-white shadow-md' : 'bg-white text-zinc-600 border-zinc-200'}`}>{cat.label}</button>
                <div className="absolute top-full left-0 mt-1 hidden group-hover/menu:block bg-white shadow-2xl rounded-lg border border-zinc-100 z-50 w-44 overflow-hidden">
                   {cat.subs.map(sub => (
                     <button key={sub} onClick={() => {setCategoriaAtiva(cat.id); setSubCategoriaAtiva(sub);}} className="w-full text-left px-5 py-3 text-[10px] font-bold hover:bg-[#F9F6F7] hover:text-[#D4AF37] border-b last:border-0 border-zinc-50">{sub}</button>
                   ))}
                </div>
             </div>
           ))}
        ...
        {carregando ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-20">
            {produtosFiltrados.map(p => (
              <ProdutoCard key={p.id} produto={p} categoriasBase={categoriasBase} abrirDetalhe={setProdutoDetalheAberto} adicionarAoCarrinho={(item:any) => setCarrinho([...carrinho, item])} setNotificacao={setNotificacao} />
            ))}
          </div>
        )}
      </section>

      <ModalDetalheProduto aberto={!!produtoDetalheAberto} produto={produtoDetalheAberto} fechar={() => setProdutoDetalheAberto(null)} adicionarAoCarrinho={(item:any) => setCarrinho([...carrinho, item])} setNotificacao={setNotificacao} categoriasBase={categoriasBase} />
      <ModalMedidas aberto={guiaAberto} fechar={() => setGuiaAberto(false)} />

      {/* FOOTER */}
      <footer className="bg-[#611F3A] py-16 px-6 text-white text-center border-t border-zinc-100">
        <h3 className="text-3xl font-serif italic mb-8 text-[#D4AF37]">Closet Dellas</h3>
        <div className="flex flex-col md:flex-row justify-center gap-10 text-xs font-bold uppercase tracking-widest opacity-70 mb-12">
          <a href="#">Trocas</a>
          <a href="#">Privacidade</a>
          <a href="#">Instagram</a>
          <a href="#">WhatsApp</a>
        </div>
        <div className="text-[9px] opacity-40 uppercase tracking-[0.3em]">© 2026 Closet Dellas • Miguel Pereira - RJ</div>
      </footer>
    </main>
  );
}