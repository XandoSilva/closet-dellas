    ...
    trocas: {
      ...
      texto: (
        <div className="space-y-4 text-sm text-zinc-600">
          ...
          <p><span className="font-bold">Defeitos:</span> Até 30 dias para comunicar falhas de fabricação.</p>
  ...
  return (
    <div className="fixed inset-0 bg-[#611F3A]/60 backdrop-blur-sm z-[10001] flex items-center justify-center p-4" onClick={fechar}>
      <div className="bg-white w-full max-w-lg p-8 rounded-3xl shadow-2xl relative animate-in zoom-in duration-300" onClick={(e) => e.stopPropagation()}>
        <button onClick={fechar} className="absolute top-6 right-6 bg-zinc-100 w-8 h-8 rounded-full flex items-center justify-center text-[#611F3A] hover:bg-zinc-200 transition-colors">
          ×
        </button>
        ...
        <button onClick={fechar} className="w-full mt-8 bg-[#611F3A] text-white py-4 rounded-full text-[10px] font-bold uppercase tracking-widest">
          Entendi
        </button>
      </div>
    </div>
...
function ModalDetalheProduto({ produto, aberto, fechar, adicionarAoCarrinho, setNotificacao, categoriasBase }) {
  ...
  const tamanhosAgrupados = produto.grade
    ...
    .reduce((acc, current) => {
      ...
      if (!existente) acc.push({ ...current });
      else existente.qtd += current.qtd;
      return acc;
    }, []);

  const handleAddCart = () => {
    if (produto.cores && produto.cores.length > 0 && !cor) {
      return setNotificacao('Selecione uma cor para sua peça!');
    }

    if (!tamanho) {
      return setNotificacao('Por favor, selecione um tamanho disponível!');
    }
...
function ProdutoCard({ produto, categoriasBase, adicionarAoCarrinho, setNotificacao, abrirDetalhe }) {
  ...
  const tamanhosAgrupados = produto.grade
    .filter((g) => !cor || g.cor === cor)
    .reduce((acc, current) => {
      const existe = acc.find((item) => String(item.tam).trim() === String(current.tam).trim());
  ...
  return (
    <div className="group flex flex-col bg-white p-4 rounded-[2rem] border border-transparent transition-all duration-500 hover:border-zinc-100 hover:shadow-[0_30px_60px_rgba(97,31,58,0.08)] relative animate-in fade-in duration-700">
      {produto.temPromo && !esgotado && (
        <span className="absolute top-7 right-7 bg-red-600 text-white text-[9px] uppercase tracking-[0.2em] font-bold px-4 py-2 rounded-full z-30 shadow-lg">Oferta</span>
      )}
      {produto.ehNovidade && !esgotado && !produto.temPromo && (
      ...
      <div
        className={`relative aspect-[3/4] w-full rounded-2xl overflow-hidden mb-6 shadow-sm ${!produto.imagens || produto.imagens.length === 0 ? 'cursor-default' : 'cursor-pointer'}`}
        onClick={!produto.imagens || produto.imagens.length === 0 ? null : () => abrirDetalhe(produto)}
      >
        <CarrosselProduto imagens={produto.imagens} nome={produto.nome} />
        ...
        {(!produto.imagens || produto.imagens.length === 0) && (
          <div className="absolute inset-0 bg-white/40 backdrop-blur-[2px] z-20 flex items-center justify-center pointer-events-none overflow-hidden">
            <div className="bg-[#611F3A] text-white py-3 transform -rotate-45 font-bold uppercase tracking-[0.4em] text-[10px] shadow-2xl border-y border-[#D4AF37]/50 w-[160%] text-center">
              Em Breve
            </div>
          </div>
        )}
      ...
      <div className="text-left px-2 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-2">
          <p className="text-[11px] text-[#D4AF37] uppercase tracking-[0.2em] font-bold">
            {categoriasBase.find((c) => c.id === produto.categoria)?.label || 'DIVERSOS'}
          </p>
        ...
        <div className="flex gap-2 mt-auto">
          {!produto.imagens || produto.imagens.length === 0 ? (
            <button disabled className="w-full bg-zinc-50 text-zinc-400 py-4 rounded-full text-[11px] uppercase tracking-[0.2em] font-bold shadow-none cursor-not-allowed border border-zinc-100">
              Aguarde o Lançamento
            </button>
          ) : (
            <>
              <button onClick={() => abrirDetalhe(produto)} className="md:hidden flex-1 bg-zinc-900 text-white py-4 rounded-full text-[11px] uppercase font-bold shadow-lg hover:bg-black transition-colors">
                Detalhes
              </button>
              <button
                onClick={handleQuickAdd}
                disabled={esgotado}
                className="flex-1 bg-[#611F3A] text-white py-4 rounded-full text-[11px] uppercase tracking-[0.2em] font-bold shadow-lg hover:bg-[#D4AF37] transition-all active:scale-95 disabled:bg-zinc-200 disabled:shadow-none"
              >
                ...
                return (
                  <div key={index} className="flex gap-6 items-center animate-in fade-in slide-in-from-right-8">
                    <div className="w-20 h-28 flex-shrink-0 rounded-xl overflow-hidden shadow-sm">
                      <img src={otimizarImg(imgCart)} className="w-full h-full object-cover" />
                    </div>
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-[#D4AF37] mt-1.5 font-bold uppercase tracking-widest">
                        {item.corSelecionada ? `${item.corSelecionada} • ` : ''}Tam {item.tamanhoSelecionado}
                      </p>
                      <p className={`text-sm font-serif italic mt-2 ${item.temPromo ? 'text-red-600 font-bold' : 'text-[#611F3A]'}`}>
                        R$ {Number(precoExibicao).toFixed(2)}
                      </p>
                    </div>

                    <button onClick={() => remover(index)} className="w-8 h-8 rounded-full bg-zinc-50 flex items-center justify-center text-zinc-300 hover:bg-zinc-100 hover:text-red-400 transition-all">
                      ×
                    </button>
                  </div>
                );
          ...
          <div className="p-8 bg-zinc-50 border-t border-zinc-100">
            {passoCheckout === 1 ? (
              <div>
                ...
                <button onClick={() => setPassoCheckout(2)} className="w-full bg-[#611F3A] text-white py-5 rounded-full text-[11px] uppercase tracking-[0.3em] font-bold shadow-2xl hover:bg-[#D4AF37] transition-all transform active:scale-95">
                  AVANÇAR
                </button>
              </div>
            ) : (
              <div className="space-y-3">
                <button
                  ...
                  className="w-full bg-[#25D366] text-white py-4 rounded-full text-[10px] font-bold uppercase tracking-widest shadow-lg hover:bg-[#1DA851] transition-all transform active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50"
                >
                  <span>📲</span> WhatsApp
                </button>
                <button
                  ...
                  disabled={!nomeDella.trim() || !cidadeDella.trim()}
                  className="w-full bg-[#0088cc] text-white py-4 rounded-full text-[10px] font-bold uppercase tracking-widest shadow-lg hover:bg-[#0077b3] transition-all transform active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50"
                >
...
export default function Home() {
  ...
  const CLARITYID = 'w2dhylfktb';
  const GA4ID = 'G-P13JKPTP4E';

  const SHEETCSVURL =
    'https://docs.google.com/spreadsheets/d/e/2PACX-1vTiXIKGK6tBbAxErk8F6eCYoJPmb7FjK7Yo-UDDVlraJmQ-8x3ea2EtR4dS9hHkqBbGHEnPZEC6-64/pub?gid=1773071955&single=true&output=csv';
  const SHEETBANNERSURL =
    'https://docs.google.com/spreadsheets/d/e/2PACX-1vTiXIKGK6tBbAxErk8F6eCYoJPmb7FjK7Yo-UDDVlraJmQ-8x3ea2EtR4dS9hHkqBbGHEnPZEC6-64/pub?gid=1879558148&single=true&output=csv';

  const bannersFallback = [
    {
      imagem: 'https://images.unsplash.com/photo-1490481651871-ab68de25d43d?q=80&w=1600&h=800&fit=crop',
      tag: 'Curadoria de Luxo',
      tituloPrincipal: 'A elegância que',
      tituloDestaque: 'você merece.',
    },
    {
      imagem: 'https://images.unsplash.com/photo-1483985988355-763728e1935b?q=80&w=1600&h=800&fit=crop',
      tag: 'Novidades Chegando',
      tituloPrincipal: 'Nova coleção',
      tituloDestaque: 'selecionada a dedo.',
    },
  ];
  ...
  const categoriasBase = [
    { id: 'vestidos', label: 'VESTIDOS', subs: ['Longo', 'Midi', 'Curto', 'T-shirt', 'Regata', 'Básico', 'Crochê', 'Renda', 'Corset', 'Amarração', 'Pantalona', 'Alfaiataria', 'Jeans', 'Tricô', 'Sobretudo', 'Jaqueta', 'Bobojaco', 'Cintos'] },
    { id: 'blusas', label: 'BLUSAS', subs: ['Longo', 'Midi', 'Curto', 'T-shirt', 'Regata', 'Básico', 'Crochê', 'Renda', 'Corset', 'Amarração', 'Pantalona', 'Alfaiataria', 'Jeans', 'Tricô', 'Sobretudo', 'Jaqueta', 'Bobojaco', 'Cintos'] },
    { id: 'cropped', label: 'CROPPED', subs: ['Longo', 'Midi', 'Curto', 'T-shirt', 'Regata', 'Básico', 'Crochê', 'Renda', 'Corset', 'Amarração', 'Pantalona', 'Alfaiataria', 'Jeans', 'Tricô', 'Sobretudo', 'Jaqueta', 'Bobojaco', 'Cintos'] },
    { id: 'calcas', label: 'CALÇAS', subs: ['Longo', 'Midi', 'Curto', 'T-shirt', 'Regata', 'Básico', 'Crochê', 'Renda', 'Corset', 'Amarração', 'Pantalona', 'Alfaiataria', 'Jeans', 'Tricô', 'Sobretudo', 'Jaqueta', 'Bobojaco', 'Cintos'] },
    { id: 'body', label: 'BODY', subs: ['Longo', 'Midi', 'Curto', 'T-shirt', 'Regata', 'Básico', 'Crochê', 'Renda', 'Corset', 'Amarração', 'Pantalona', 'Alfaiataria', 'Jeans', 'Tricô', 'Sobretudo', 'Jaqueta', 'Bobojaco', 'Cintos'] },
    { id: 'conjuntos', label: 'CONJUNTOS', subs: ['Longo', 'Midi', 'Curto', 'T-shirt', 'Regata', 'Básico', 'Crochê', 'Renda', 'Corset', 'Amarração', 'Pantalona', 'Alfaiataria', 'Jeans', 'Tricô', 'Sobretudo', 'Jaqueta', 'Bobojaco', 'Cintos'] },
    { id: 'saias', label: 'SAIAS', subs: ['Longo', 'Midi', 'Curto', 'T-shirt', 'Regata', 'Básico', 'Crochê', 'Renda', 'Corset', 'Amarração', 'Pantalona', 'Alfaiataria', 'Jeans', 'Tricô', 'Sobretudo', 'Jaqueta', 'Bobojaco', 'Cintos'] },
    { id: 'shorts', label: 'SHORTS', subs: ['Longo', 'Midi', 'Curto', 'T-shirt', 'Regata', 'Básico', 'Crochê', 'Renda', 'Corset', 'Amarração', 'Pantalona', 'Alfaiataria', 'Jeans', 'Tricô', 'Sobretudo', 'Jaqueta', 'Bobojaco', 'Cintos'] },
    { id: 'casacos', label: 'CASACOS', subs: ['Longo', 'Midi', 'Curto', 'T-shirt', 'Regata', 'Básico', 'Crochê', 'Renda', 'Corset', 'Amarração', 'Pantalona', 'Alfaiataria', 'Jeans', 'Tricô', 'Sobretudo', 'Jaqueta', 'Bobojaco', 'Cintos'] },
    { id: 'acessorios', label: 'ACESSÓRIOS', subs: ['Longo', 'Midi', 'Curto', 'T-shirt', 'Regata', 'Básico', 'Crochê', 'Renda', 'Corset', 'Amarração', 'Pantalona', 'Alfaiataria', 'Jeans', 'Tricô', 'Sobretudo', 'Jaqueta', 'Bobojaco', 'Cintos'] },
  ...
  useEffect(() => {
    const fetchDados = async () => {
      ...
      const acharColuna = (headerMap: Record<string, number>, aliases: string[], fallback?: number) => {
      ...
      const valorColuna = (cols: string[], headerMap: Record<string, number>, aliases: string[], fallback?: number) => {
      ...
      const extrairImagensLinha = (cols: string[], headerMap: Record<string, number>) => {
        const candidatas = [
          ...
          valorColuna(cols, headerMap, ['imagem principal', 'foto principal', 'imagem', 'foto', 'principal'], 18),
        ];
      ...
      try {
          ...
          if (rowsBanners.length > 1) {
            ...
            const parsedBanners = rowsBanners
              ...
              .map((row) => {
                ...
                const imagem = valorColuna(cols, headerMapBanners, ['imagem', 'image', 'banner', 'url imagem', 'url', 'foto'], 0);
        ...
        const headerTemCaraDeCabecalho = headerRow.some((h) =>
          ['sku', 'categoria', 'nome', 'estoque', 'preco', 'preço', 'descricao', 'descrição', 'imagem', 'foto'].includes(normalizar(h))
        );
        ...
        const rawData = rows
          ...
          .map((row) => {
            ...
            const skuAgrupador = valorColuna(cols, headerMap, ['sku agrupador', 'sku pai', 'ref agrupadora', 'referencia agrupadora', 'sku'], 0);
            const skuCompleto = valorColuna(cols, headerMap, ['sku completo', 'sku variacao', 'sku variação', 'codigo sku', 'codigo', 'código'], 1);
            const nome = valorColuna(cols, headerMap, ['nome', 'produto', 'nome produto', 'descricao produto'], 3);
            const categoria = valorColuna(cols, headerMap, ['categoria', 'categoria principal'], 4);
            const subcategoria = valorColuna(cols, headerMap, ['subcategoria', 'sub categoria', 'sub-categoria'], 5);
            const cor = valorColuna(cols, headerMap, ['cor', 'cor produto'], 6);
            const tamanho = valorColuna(cols, headerMap, ['tamanho', 'tam', 'grade'], 7);
            const estoque = parseInteiro(valorColuna(cols, headerMap, ['estoque', 'qtd estoque', 'quantidade', 'saldo'], 10));
            const precoReal = parseValor(valorColuna(cols, headerMap, ['preço sugerido real', 'preco sugerido real', 'preço real', 'preco real', 'real', 'preco', 'preço'], 11));
            const precoPromoSugerido = parseValor(valorColuna(cols, headerMap, ['preço promo sugerido', 'preco promo sugerido', 'preço promo', 'preco promo', 'promo'], 12));
            const descricao = valorColuna(cols, headerMap, ['descricao', 'descrição', 'descricao curta', 'descrição curta'], 17);
  ...
  const produtosFiltrados = todosProdutos.filter((p) => {
    ...
    const matchCategoria = categoriaAtiva === 'todas' ? true : p.categoria === categoriaAtiva;
    ...
    <main className="min-h-screen bg-[#f5e3ed] text-zinc-900 font-sans relative overflow-x-hidden pb-24 md:pb-0">
      ...
      >
        <svg fill="currentColor" viewBox="0 0 24 24" className="w-8 h-8">
          <path d="M12.031 2.007a9.969 9.969 0 00-8.5 15.228l-1.468 5.362 5.485-1.438a9.964 9.964 0 004.483 1.066h.004c5.5 0 9.975-4.475 9.975-9.974 0-2.666-1.038-5.17-2.923-7.054A9.92 9.92 0 0012.031 2.007z" />
        </svg>
      </a>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 py-5 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="relative w-full md:w-[450px]">
            <input
              ...
              className="w-full bg-zinc-50 border-none rounded-full px-12 py-3.5 text-xs focus:ring-2 focus:ring-[#611F3A]/50 outline-none transition-all placeholder:text-zinc-300 shadow-inner"
            />
            <span className="absolute left-5 top-1/2 -translate-y-1/2 opacity-20">⌕</span>
            {busca && (
              <button onClick={() => setBusca('')} className="absolute right-5 top-1/2 -translate-y-1/2 text-zinc-300 hover:text-[#611F3A]">
                ×
              </button>
            )}
          </div>
          ...
          <div className="hidden md:flex gap-8 items-center">
            <button onClick={() => setGuiaAberto(true)} className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#611F3A] hover:text-[#D4AF37] transition-colors">
              Guia de Medidas
            </button>
      ...
      <section
        className="bg-gradient-to-r from-[#611F3A] to-[#8a2b52] py-10 px-6 text-white text-center cursor-pointer hover:opacity-95 transition-opacity"
      ...
      >
        <div className="max-w-3xl mx-auto flex flex-col items-center">
          ...
          <button className="text-[10px] font-bold uppercase tracking-[0.3em] bg-white text-[#611F3A] px-6 py-2 rounded-full hover:bg-[#D4AF37] hover:text-white transition-all shadow-lg">
            Saiba Como Funciona
          </button>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto pt-16 px-6">
          ...
          <div className="flex flex-wrap justify-center gap-3 w-full">
            {categoriasBase.map((cat) => {
              const subsComProduto = cat.subs?.filter((sub) =>
                todosProdutos.some((p) => p.categoria === cat.id && (p.subcategoria || '').toLowerCase().trim() === sub.toLowerCase().trim())
              );

              return (
                <div key={cat.id} className={`relative group/menu ${menuAbertoCat === cat.id ? 'z-50' : 'z-10'}`}>
                  {menuAbertoCat === cat.id && <div className="fixed inset-0 z-[190] bg-transparent" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setMenuAbertoCat(null); }} />}
                  <button
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      ...
                      <div className="bg-white shadow-[0_30px_60px_rgba(0,0,0,0.2)] rounded-2xl border border-zinc-100 overflow-hidden">
                        {subsComProduto.map((sub) => (
                          <button
                            ...
                            className={`w-full text-center px-6 py-4 text-[10px] font-bold uppercase tracking-widest active:bg-zinc-100 transition-colors border-b last:border-0 border-zinc-50 ${
                              subCategoriaAtiva === sub ? 'text-[#D4AF37] bg-zinc-50' : 'text-zinc-500'
                            }`}
                          >
                            {sub}
                          </button>
        ...
        {carregando ? (
          ...
          <NoResults mensagem={busca ? `Não encontramos nada para "${busca}". A coleção está sendo atualizada! Volte em breve.` : 'Nossa coleção está sendo atualizada agora. Volte em instantes!'} />
        ) : (
      ...
      <footer className="bg-[#611F3A] pt-24 pb-12 px-6 md:px-12 text-white">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-16 items-start text-center md:text-left">
          <div className="md:col-span-1">
            ...
            <p className="text-sm font-light leading-relaxed opacity-80 mb-8 md:max-w-xs">
              Sua curadoria exclusiva das melhores tendências, unindo sofisticação e preço justo para mulheres reais.
            </p>
          </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-lg border-t border-zinc-100 px-8 py-3 flex justify-between items-center z-[9000] shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
        <button onClick={() => { setCategoriaAtiva('todas'); setSubCategoriaAtiva(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="flex flex-col items-center gap-1">
        ...
        <button onClick={() => setCarrinhoAberto(true)} className={`relative flex flex-col items-center gap-1 transition-transform ${sacolaPulse ? 'scale-110' : ''}`}>
